...
INSERT INTO employee (id, name, phone, email, profession, age, gender, status, createtime) VALUES
...
#practice the creating of index in the Linux SQL
create index index_name on employee(name);
create unique index  index_phone on employee(phone);
create unique index  index_email on employee(email);
create index  index_profession_age_status on employee(profession,age,status);
drop index  index_email on employee;
show global status like"Com_______";
select employee.name from employee;
-- 开启慢查询日志 (Turn on the slow query log)
SET GLOBAL slow_query_log = 'ON';

-- 设置慢查询的时间阈值为 2 秒 (Set the threshold to 2 seconds)
